...
class Pixoo:
    ...
    def __init__(
        self, address, size=64, debug=False, refresh_connection_automatically=True, timeout=10
    ):
    ...
    def set_screen(self, on=True):
        response = requests.post(
            self.__url,
            json.dumps({"Command": "Channel/OnOffScreen", "OnOff": 1 if on else 0}),
        )
        data = response.json()
        if data["error_code"] != 0:
            self.__error(data)
    ...
    def get_state(self):
        response = requests.post(
            self.__url, json.dumps({"Command": "Channel/GetAllConf"})
        )
        data = response.json()
    ...
    def __reset_counter(self):
        ...
        response = requests.post(
            self.__url, json.dumps({"Command": "Draw/ResetHttpGifId"})
        )
        data = response.json()
        if data["error_code"] != 0:
            self.__error(data)
