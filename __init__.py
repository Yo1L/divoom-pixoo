...
async def async_setup(hass: HomeAssistant, config: dict):
    ...
    conf = config[DOMAIN]
    hass.data[DOMAIN] = {"ip_address": conf["ip_address"], "entities": []}
    ...
    hass.async_create_task(
        discovery.async_load_platform(
            hass, "light", DOMAIN, {"ip_address": conf["ip_address"]}, config
        )
    )
    ...
    async def async_show_message(service: ServiceCall):
        ...
        for entity in hass.data[DOMAIN].get("entities", []):
            if entity.entity_id == entity_id:
                await entity.async_show_message(
                    messages, positions, colors, fonts, images, image_positions
                )
                break
